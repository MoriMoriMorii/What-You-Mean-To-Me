...
label poemresponse_start:
    ...
    label poemresponse_start2:
        ...
        menu:
            ...
            "Lily" if not l_readpoem and persistent.lily:
                # This variable sets that you have read Sayori's poem.
                $ l_readpoem = True
                if not lily_firstread and poemsread == 0:
                    $ lily_firstread = True
                    "I feel like sharing my poem with Lily first today."
                    "She's new to the club afterall, and I want to make her feel welcome!"
                # This call statement calls Sayori's poem response script.
                call poemresponse_lily
...
label poemresponse_lily:
    scene bg club_day
    show lily 1a zorder 2 at t11
    with wipeleft_scene

    if l_poemappeal[chapter - 1] < 0:
        $ poemopinion = "bad"
    elif l_poemappeal[chapter - 1] > 0:
        $ poemopinion = "good"

    $ nextscene = "ch" + pt + str(chapter) + "_l_" + poemopinion
    call expression nextscene
    
    # This if statement checks if we are not skipping the poems to call the
    # end of the poem responses for Sayori depending on the chapter.
    if not skip_poem:
        $ nextscene = "ch" + pt + str(chapter) + "_l_end"
        call expression nextscene
    return
...
label m_lily_1:
    m 2a "I like this one!"
    m "Appealing to our new member?"
    mc "Eh?"
    m 2d "You want to make them feel welcome, don't you."
    m "I wouldn't be surprised if you had some things in common."
    mc "Ah, well..."
    m 2a "...But in any case, Lily's writing seems to be more ethereal than most."
    m "I can tell that she likes to talk about the unknown, and what it can bring."
    m "Who knew that someone so outgoing could be so mysterious?"
    mc "Yeah... That's totally unexpected."
    m 2j "Well, to each their own~"
    m 2a "And you shouldn't be afraid to experiment a little bit, either."
    return
...
label ch1_l_good:
    l 1b "Woahhh!"
    l "How is yours so cool!"
    mc "I'm not too terribly good yet though, so I still have some tweeks I want to make to my style."
    l "How do you have tweeks to make when it's already perfect!"
    mc "Eh?"
    l "The suspense, the drama, the realism, it all works so wel!"
    return
label ch1_l_med:
    l 1a "It's pretty cool!"
    l "What made you choose this topic?"
    mc "I dunno, I just usually write about whatever."
    l "Coolio."
label ch1_l_bad:
    l 1a "That's a poem alright!"
    "What does she mean by that??"
label ch1_l_end:
    $ show_poem(poem_l1)
    l "I just started writing so it's not gonna be too good..."
    mc "What do you mean? This is really good."
    return
