...
image l_sticker smile:
    "l_sticker"
    pause 0.1
    xoffset lilyOffset xzoom lilyZoom
    "gui/poemgame/l_sticker_2.png"
...
init python:
    ...
    class NewPoemWord:
        def __init__(self, word, sPoint, nPoint, yPoint, lPoint, glitch=False):
            self.word = word
            self.sPoint = sPoint
            self.nPoint = nPoint
            self.yPoint = yPoint
    ...
    full_new_wordlist = []
    with renpy.file('MASpoemwords.txt') as wordfile:
    ...
    def draw_words(cTimer):
        ...
        for j in range(2):
            ...
            for i in range(5):
                ...
                ui.textbutton(word.word, clicked=Function(select_doki, word, cTimer, _update_screens=False), text_style="poemgame_text", xpos=x, ypos=i * 56 + ystart)
            ui.close()
...
label new_m_poem(transition=True):
    ...
    show l_sticker at sticker_rightest
    if transition:
    ...
    python:
        ...
        unsorted_pointlist = {"sayori": sPointTotal, "natsuki": nPointTotal, "yuri": yPointTotal, "lily": lPointTotal}
